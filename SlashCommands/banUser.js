    ...
    async run(message, args, client) {
        try {
            ...
            if(await member.roles.cache.has('923464931628683264')) {
                ...
                let result = await banUser(args[0].value)
                result = await JSON.parse(result)
    
                if (result.result == "User banned") {
    
                    const fetchedMember = await message.guild.members.fetch(result.dcID);
    
                    const embed = new Discord.EmbedBuilder()
    
                    .setTitle(`Banned user ${fetchedMember.user.tag}`)
                    .setAuthor({
                ...
                try {
                    ...
                    sendErrorDC(client, message, "banUser", error)
                    console.log('something went wrong, user still banned.')
                }
                } else {
                ...
                console.log(error)
                sendErrorDC(client, message, "banUser", error)
                message.reply('Something went wrong, user still banned.')
            }            
    }
});